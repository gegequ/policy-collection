...
## 🥇 黄金/贵金属配置建议
基于当日政策信号和国际宏观背景，给出黄金配置方向：
- 配置建议：增配 / 中性持有 / 减配
- 核心逻辑：（货币政策、地缘政治、通胀预期、央行购金等角度的分析）
- 风险提示：

## 📉 债券市场研判
- 利率债：收益率走势判断（下行/震荡/上行）+ 逻辑
- 信用债：信用环境判断（改善/稳定/恶化）+ 逻辑
- 可转债：当前性价比判断

## 💰 指数基金配置参考（不构成投资建议）

| 资产类别 | 建议方向 | 参考标的（示例） | 逻辑简述 |
|----------|---------|-----------------|----------|
| 宽基指数 | 增配/中性/减配 | 沪深300/中证500/创业板 | 一句话逻辑 |
| 行业ETF | 增配/中性/减配 | 证券/新能源/半导体/医药/消费等 | 根据当日信号推荐1-3个方向 |
| 债券基金 | 增配/中性/减配 | 纯债/二级债基 | 一句话逻辑 |
| 黄金ETF | 增配/中性/减配 | 黄金ETF | 一句话逻辑 |
| 货币基金 | 增配/中性/减配 | 货基/短债 | 一句话逻辑 |

## ⚠️ 风险提示
（需要持续跟踪的政策动向或潜在风险）

## 💡 综合配置思路
（2-3句话总结当前环境下的大类资产配置方向，不构成具体投资建议）
"""
