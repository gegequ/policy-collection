...
def compute_xwlb_monthly(db, report_dir: str = "./reports") -> str:
    """提取本月新闻联播标题，生成月度汇总文件并返回AI分析摘要。

    按月份分组，生成 reports/xwlb/YYYY-MM.md 月度汇总文件。
    返回给AI的摘要包含高频主题统计。
    """
    from collections import Counter
    ...
    monthly_path = _os.path.join(report_dir, "xwlb", f"{current_month}.md")

    all_items = []
    # 读取本月所有日期
    for i in range(31):
        d = (now - _td(days=i)).strftime("%Y-%m-%d")
        if not d.startswith(current_month):
            continue
        try:
            arts = db.get_articles_by_date(d)
            xwlb = [a for a in arts if a.source == "新闻联播"]
            for a in xwlb:
                all_items.append({"date": d, "title": a.title})
        except Exception:
            pass

    if len(all_items) < 5:
        return "（本月新闻联播数据不足，暂时无法生成月度趋势）"

    # 生成月度汇总文件
    _os.makedirs(_os.path.dirname(monthly_path), exist_ok=True)
    with open(monthly_path, "w", encoding="utf-8") as f:
        f.write(f"# 📺 新闻联播月度汇总 · {current_month}\n\n")
        f.write(f"共 {len(all_items)} 条\n\n")
        # 按日期分组
        by_date = {}
    ...
    # 生成AI摘要
    lines = [
        f"\n## 📺 新闻联播 {current_month} 月度汇总",
        f"本月已采集 {len(all_items)} 条，日均 {len(all_items)//max(1, len(by_date))} 条。",
        f"完整月度文件：{monthly_path}",
        "",
        "请在报告中基于以上月度数据，分析本月高频主题和趋势。",
    ]
    return "\n".join(lines)
...
6. 板块分析必须包含：政策面 + 估值面 + 资金面，缺一不可。
   - ⚠️ **估值面**：从当日采集的文章中寻找 PE/分位引用。如果某篇文章提到了"沪深300 PE 14倍 88%分位"，引用该数据并标注出处（文章链接）。当日文章中没有的板块，写「当日采集文章中未找到该板块PE分位数据」。禁止凭记忆编造。
...
8. 每个板块给出短期（1-4周）和长期（3-12月）走势预测，注明预测依据。
9. **信息源权重**（极其重要）：
...
## 📺 新闻联播信号解读
上方数据中如有新闻联播来源文章（约15条），必须基于当日播报内容分析以下信号。如确实没有文字稿（19:00前采集），说明"新闻联播尚未播出"。
...
## 📺 新闻联播月度趋势（本月）
基于本月新闻联播标题统计，分析高频主题和趋势方向。
- 本月出现≥3次的高频主题，标注次数+出现日期
- 判断"持续升温/平稳推进/逐步降温"
- 映射到对应板块

## 📈 板块热度与趋势
覆盖 TOP5 + 消费 + 医药 + 创新药 + 半导体 + 科技 + 军工航天 + 光伏 + 新能源 + 电力 + 建材 + 有色金属 + 机器人，逐个简述：
- **板块**：热度N次，趋势📈/📉/📊
- 短期（1-4周）：预测方向 + 依据
- 长期（3-12月）：预测方向 + 依据

## 📊 重点板块评估
逐板块深度分析（TOP5 + 消费 + 医药 + 创新药 + 半导体 + 科技 + 军工航天 + 光伏 + 新能源 + 电力 + 建材 + 有色金属 + 机器人），格式：
- **板块**：利好/利空/中性，程度强/中/弱，热度见上方数据表
- 📊 行情：必须查上方实时行情表。有该板块→写具体价格+涨跌幅。找不到→写「行情表中无此板块」。
  板块→行情映射：金融→中证银行/证券，科技→科创50，消费→中证消费，医药→中证医药，创新药→生物医药ETF(净值)，军工→中证军工，半导体→半导体指数，新能源→中证新能源/CS新能车，数字经济→数字经济ETF(净值)，机器人→机器人ETF(净值)，光伏→光伏ETF(净值)，建材→建材ETF(净值)，电力→电力ETF(净值)，有色金属→有色金属指数，基建→中证基建
  ⚠️ ETF净值是元/份，不是指数点位。比如"建材ETF¥0.64"是每份0.64元，不是指数5626点。引用ETF数据必须写完整单位"元/份"。
- 近半月/近三月走势：（基于行情数据+热度趋势判断，不写具体历史价格区间）
- ⚠️ 估值：从当日文章找PE分位引用→标注出处链接。文章中没有的→写「未找到PE数据」。不许编。
...
- 上海金估算价：见上方估算值
- ⚠️ 注：美元指数（DXY）、10年期美债收益率和通胀预期均为实时数据（来自新浪财经/FRED），请直接引用上方行情数据中的具体数值，无需使用训练知识估算
...
要求覆盖的板块：TOP5 + 消费 + 医药 + 创新药 + 半导体 + 军工航天 + 光伏 + 新能源 + 电力 + 建材 + 有色金属 + 机器人。
黄金和债券也请分别输出一行（方向用 利好/利空/中性，强度用 强/中/弱）。
"""


def extract_key_sentences(text: str, max_sentences: int = 3) -> str:
    """从文章正文中提取与板块关键词最相关的句子。

    用于将 2000+ 字正文压缩为 2-3 句高信号原文引用，
    减少 AI 输入 token 的同时保留原文具体措辞。

    算法：按句切分 → 用 SECTOR_KEYWORDS + NEGATIVE_KEYWORDS 打分
    → 返回得分最高的 N 句。

    Args:
        text: 文章正文全文。
        max_sentences: 最多返回句数。

    Returns:
        用「 ▸ 」连接的原文关键句，正文为空返回空字符串。
    """
    import re as _re
    ...
    # 构建打分关键词集合（板块名 + 所有关键词）
    all_keywords: Dict[str, int] = {}  # keyword → score weight
    for sector, keywords in SECTOR_KEYWORDS.items():
        for kw in keywords:
            all_keywords[kw] = all_keywords.get(kw, 0) + 1
    for sector, keywords in NEGATIVE_KEYWORDS.items():
        for kw in keywords:
            all_keywords[kw] = all_keywords.get(kw, 0) + 2  # 负向信号加权

    # 对每句打分
    scored: list[tuple[int, str]] = []
    for s in sentences:
        score = sum(w for kw, w in all_keywords.items() if kw in s)
        if score > 0:
...
def format_stats_for_ai(stats: Dict, articles: List[Article]) -> str:
    ...
    required_sectors = ["金融", "科技", "数字经济", "制造", "消费", "贵金属", "医药", "创新药",
                        "半导体", "军工航天", "光伏", "新能源", "电力", "建材", "有色金属", "机器人", "基建", "环保"]
    ...
    # 缓存每篇文章的负向信号，避免在触发文章查找时重复调用 extract_negative_signals
    article_neg_cache: list[dict[str, int]] = []
    for a in articles:
        neg_signals = extract_negative_signals(a.title + " " + a.summary)
        article_neg_cache.append(neg_signals)
        for sector, count in neg_signals.items():
            neg_counter[sector] += count
    if neg_counter:
        lines.append("\n## ⚠️ 负向信号预警")
        lines.append("（系统关键词扫描结果，非 AI 判断。引用时必须标注具体文章标题+URL，禁止写「出处：AI分析系统」）")
        for sector, count in neg_counter.most_common(10):
            trigger_articles = []
            for i, a in enumerate(articles):
                if sector in article_neg_cache[i]:
                    trigger_articles.append(f"[{a.title[:25]}...]({a.url})")
                if len(trigger_articles) >= 2:
                    break
            triggers = "；".join(trigger_articles) if trigger_articles else "（无具体文章）"
            lines.append(f"  - {sector}：{count} 次 — {triggers}")

    lines.append(f"\n## 📋 一手政策来源（政府官网）— {len(tier1)} 篇，分析以这些为准")
    for i, a in enumerate(tier1[:12], 1):
        lines.append(f"{i}. [{a.source}]({a.url}) {a.title}")
        key_lines = extract_key_sentences(a.summary, max_sentences=3)
        if key_lines:
            lines.append(f"   > {key_lines}")
    ...
    for i, a in enumerate(tier2[:8], 1):
        lines.append(f"{i}. [{a.source}]({a.url}) {a.title}")
        key_lines = extract_key_sentences(a.summary, max_sentences=2)
        if key_lines:
            lines.append(f"   > {key_lines}")
        tag_str = ", ".join(a.tags) if a.tags else "—"
        lines.append(f"   #{tag_str}")
