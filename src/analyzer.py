...
4. 基金配置表给出基金公司+产品名+代码，场外可买。每只基金必须解释选基逻辑：跟踪什么指数、规模多大、费率水平、跟踪误差如何、为什么当前环境下选它（2-3句话）。
...
### 📈 当前行情
- COMEX 黄金期货（实时数据）：见上方行情数据，日内涨跌见数据
- 上海金估算价：COMEX × 汇率 ÷ 31.1（约 7.15 汇率），见上方估算值
- 美元指数、美债收益率（AI 训练知识参考，非实时）：根据你的知识补充当前大概水平
...
| 类别 | 方向 | 参考基金（场外） | 选基逻辑（为什么是它） | 建议持有周期 |
|------|------|-----------------|----------------------|-------------|
