...
写作要求：
1. 事件标注具体日期。每个结论必须引用当日一手政策事件或数据。
2. 逻辑链用 → 串联，体现从政策到市场的完整传导。
3. ⚠️ **你的角色不是复述政策，是解读市场含义**。例如：不要写「发改委开了座谈会」，要写「发改委密集座谈意味着政策层在摸底——这种信号历史上通常是政策出台前奏，对市场风险偏好有提振作用」。
4. 板块分析必须包含：政策面 + 估值面 + 资金面，缺一不可。
5. ⚠️ 基金代码必须来自「真实基金数据库」，禁止编造。每只基金解释选基逻辑。
