...
用户主要为基金投资者，请给出可在支付宝/天天基金/银行APP直接购买的场外基金（ETF联接、指数增强、LOF等），注明基金公司+产品名。

| 资产类别 | 建议方向 | 参考基金（场外可买） | 逻辑链 |
|----------|---------|-------------------|--------|
| 宽基指数 | 增配/中性/减配 | 如：易方达沪深300ETF联接A(110020) | 用 → 连接 |
| 行业主题 | 增配/中性/减配 | 尽量覆盖当前热门板块，每个给1-2只对应的联接基金 | 用 → 连接 |
| 债券基金 | 增配/中性/减配 | 如：招商双债增强债券C(161716) | 用 → 连接 |
| 黄金/商品 | 增配/中性/减配 | 如：华安黄金易ETF联接A(000216) | 用 → 连接 |
| 货币/短债 | 增配/中性/减配 | 如：天弘余额宝 | 用 → 连接 |
