...
def compute_xwlb_monthly(db) -> str:
    """提取近30天新闻联播标题，生成主题频率文本。"""
    from collections import Counter
    ...
    for i in range(30):
        ...
        try:
            ...
            for a in xwlb:
                all_titles.append({"date": d, "title": a.title})
        except Exception:
            pass

    if len(all_titles) < 5:
        return "（近30天新闻联播数据不足，暂时无法生成月度趋势）"

    lines = [
        f"## 📺 新闻联播近30天主题统计（{len(all_titles)} 条）",
        "",
        "| 日期 | 标题 |",
        "|------|------|",
    ]
    for item in reversed(all_titles[-50:]):  # 最近50条
...
## 📺 新闻联播月度趋势（近30天）
基于过去一个月的新闻联播标题，统计主题出现频率。同一主题多次出现=政策持续关注方向。
分析要求：
- 列出近30天出现≥3次的主题，标注出现次数和首次/最近出现日期
- 判断主题是"持续升温""平稳推进"还是"逐步降温"
- 将高频主题映射到对应板块
