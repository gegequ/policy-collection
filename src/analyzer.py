...
6. 板块分析必须包含：政策面 + 估值面 + 资金面，缺一不可。
   - ⚠️ **估值面**：不做具体PE分位数字判断（我们没有实时PE数据源）。改为：基于板块当前指数点位和历史中枢的关系做定性判断（如"科创50 1758点 vs 近一年中枢约1600点→高于中枢"），或直接写"暂无实时估值数据，不做判断"。
...
- ⚠️ 估值：不准编造PE分位数字。只准用当前指数点位 vs 历史中枢做对比。如"4883点 vs 近一年中枢4500→高于中枢"。无数据就跳过。
