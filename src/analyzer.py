...
6. 板块分析必须包含：政策面 + 估值面 + 资金面，缺一不可。
   - ⚠️ **估值面必须量化**：禁止使用「估值偏低」「估值合理」「处于低位」等模糊词。必须写出具体 PE 值 + 历史分位数（如「沪深300 PE 15倍，近5年 65% 分位，处于中位偏上」）。如果该板块有实时指数数据，PE 和分位可从你的训练知识中补充并标注「基于AI知识」，但绝对不许编造方向（不能说高位说成低位）。
...
- **板块**：利好/利空/中性，程度强/中/弱
- 近半月走势（T-15天至T-1天，不含今日）：价格区间、涨跌幅。只能引用这15天内已发生的驱动事件（禁止用今日事件解释历史走势）。
- 近三月走势（T-90天至T-1天，不含今日）：价格区间、涨跌幅、关键拐点。只能引用这90天内已发生的事件。
- ⚠️ 价格数据来源：只用上方板块指数实时行情中的实际数据。没有实时指数覆盖的板块，写「暂无实时行情数据，不做价格判断」，禁止凭记忆编造数字。
...
### 📉 历史走势
- 近半月（T-15至T-1）：COMEX实际价格走势+驱动事件（不含今日）
- 近三月（T-90至T-1）：价格区间+关键拐点+驱动事件
...
def format_stats_for_ai(stats: Dict, articles: List[Article]) -> str:
    ...
    if neg_counter:
        lines.append("\n## ⚠️ 负向信号预警（以下板块出现利空关键词，AI 分析时需特别关注）")
