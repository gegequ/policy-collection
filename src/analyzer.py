...
SECTOR_KEYWORDS: Dict[str, List[str]] = {
    ...
    "能源": ["能源", "煤炭", "石油", "天然气"],
    "电力": [
        "电力", "电网", "火电", "水电", "核电", "风电", "光伏",
        "特高压", "输配电", "电价", "售电", "虚拟电厂", "储能",
        "电力改革", "绿电", "碳交易",
    ],
...
2. 逻辑链用 → 串联，简洁有力。
3. 首次出现的专业术语括号解释，如 MLF（中期借贷便利）、LPR（贷款市场报价利率）、信用利差（信用债与国债收益率差）等。
4. 基金配置表给出基金公司+产品名+代码，场外可买。
5. 每个板块必须给出短期（1-4周）和长期（3-12个月）两个时间维度的走势预测，并注明预测依据。
...
## 📈 板块热度与趋势
覆盖 TOP5 + 消费 + 医药 + 军工航天 + 光伏 + 电力 + 建材，逐个简述：
- **板块**：热度N次，趋势📈/📉/📊
- 短期（1-4周）：预测方向 + 依据
- 长期（3-12月）：预测方向 + 依据

## 📊 重点板块评估
逐板块深度分析（TOP5 + 消费 + 医药 + 军工航天 + 光伏 + 电力 + 建材），格式：
- **板块**：利好/利空/中性，程度强/中/弱
...
|------|------|-----------------|------|-------------|
| 宽基 | 增/中/减 | 具名基金+代码 | 一句话 | 短线/中长线 |
| 行业 | 同上 | 3-5个方向各1-2只 | 同上 | 同上 |
